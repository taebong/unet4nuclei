...
code_ver = 'v4GFP'  #this version is the same as nuclei_LEXY_analysis.py, but it uses nuclei mask generated from GFP images recorded in 447 channel. Since 447 images were taken only in Lit acquisition phase, it use the nuclei masks in the first time point of Lit phase to extract intensity of LEXY/LINuS mCherry probe in PreLit phase. PostLit phase will be ignored. 
refinement_setting = {'N':10, 'repeat': 2}   #segmentation refinement setting. 
#N=3, repeat=1 for works well most of time, and N=10, repeat=2 for dense sample 
...
#lexy_Chs = ['561','Red']
Ch_map = {'642':'642','Far-Red':'nucl','561':'LEXY','Red':'LEXY','447':'nucl'}  # Ch:ChannelName dictionary. Channel names should include 'nucl' at least  
#binning_factor = 2
...
# Use the first time-point nucleus segmentation in Lit stage for the analysis of PreLit images
df_LitT1 = df_seg.loc[(df_seg['AcqState']=='Lit') & (df_seg['T']==1)]
for i,row in df_LitT1.iterrows():
    new_row = row
    new_row['AcqState'] = 'PreLit'

    for t in np.arange(1,5):
        new_row['T'] = t
        df_seg = df_seg.append(new_row)
...
for i,row in df_seg.iterrows():
    ...
    if 'nucl' not in props_dict:
        print('nucl channel image not exist for AcqState %s, Cycle %d, Pos %d, T %d' %(row['AcqState'],row['Cycle'],row['Pos'],row['T']))
        
        if (row['AcqState'] == 'PreLit'):
            props_dict['nucl'] = measure.regionprops(label,intensity_image=np.zeros(label.shape),coordinates='rc')
        else:
            continue 
